...
public class DefaultTimeColumnRemover implements DslModifier {
    @Override
    public void modify(JsonNode rootNode, ObjectMapper mapper, Canvas.Tab.Widget widget) {
        ...
        if (rootObjectNode.has(Constants.DEFAULT_TIME_COLUMN)) {
            rootObjectNode.remove(Constants.DEFAULT_TIME_COLUMN);
        }
    }
}
