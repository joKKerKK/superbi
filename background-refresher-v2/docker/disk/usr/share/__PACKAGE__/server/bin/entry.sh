...
SVC_CMD="java -DCRYPTEX_CLIENT_ID=${CRYPTEX_CLIENT_ID} -DCRYPTEX_CLIENT_SECRET=${CRYPTEX_CLIENT_SECRET} -DCRYPTEX_AUTHN_URL=${CRYPTEX_AUTHN_URL} -Dconfig.svc.buckets=${CONFIG_BUCKET} -DCFG_SVC_BUCKET=${CONFIG_BUCKET} -DENV=${ENV} ${APP_PROPS} ${GC_PROPS} ${GC_TYPE_PROPS} ${HEAP_DUMP_PROPS} ${MEMORY_OPTS} -jar ${TARGET_JAR} server ${DW_YAML_FILE}"
